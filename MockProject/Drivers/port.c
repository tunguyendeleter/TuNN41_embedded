 ...
 *
 */
void PORT_PeripheralClockConfig(PORT_TypeDef *pPORTx)
{
    /* Enable global clock to PORTA */
    if (pPORTx == PORTA)
    {
        PCC->PORT[0] |= (1 << PCC_CGC_SHIFT);
    }
    /* Enable global clock to PORTB */
    else if (pPORTx == PORTB)
    {
        PCC->PORT[1] |= (1 << PCC_CGC_SHIFT);
    }
    /* Enable global clock to PORTC */
    else if (pPORTx == PORTC)
    {
        PCC->PORT[2] |= (1 << PCC_CGC_SHIFT);
    }
    /* Enable global clock to PORTD */
    else if (pPORTx == PORTD)
    {
        PCC->PORT[3] |= (1 << PCC_CGC_SHIFT);
    }
    /* Enable global clock to PORTE */
    else if (pPORTx == PORTE)
    {
        PCC->PORT[4] |= (1 << PCC_CGC_SHIFT);
    }
}
 ...
 *
 */
void PORT_Pin_Alt_Config(PORT_TypeDef *pPORTx, uint8 PinNumber, uint8 Alternate)
{
    /* Set pin MUX as GPIO */
    pPORTx->PCR[PinNumber] &= ~(PORT_PCR_MUX_MASK);
    pPORTx->PCR[PinNumber] |= (Alternate << PORT_PCR_MUX_SHIFT);
}

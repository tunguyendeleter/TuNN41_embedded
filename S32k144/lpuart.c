...
{
    /*Setting baud rate by use bits SBR: Baud Rate Modulo Divisor  and OSR: Oversampling Ratio*/
    LPUARTx->BAUD = (SBR << 0); /*Baud rate = baud clock / ((OSR+1) * SBR)*/

    LPUARTx->BAUD |= (OSR << 24);

    /*Setting Frame*/
    LPUARTx->BAUD |= (0U << 13); /*Setting one stop bit - config 1U if want 2 stop bit*/

    /*At this line, you can config 2 more bits: CTRL: 8 or 7 bit char and parity or non-parity*/
    ...
    LPUARTx->CTRL |= (Rx_Status << 18); /*Enable or Disable Receiver*/

    LPUARTx->CTRL |= (Tx_Status << 19); /*Enable or Disable Transmiter*/
}
...
void LPUART_ReceiveString(LPUART_TypeDef* LPUARTx, char *ReceiveData){
	while (! (LPUARTx->STAT & (1U << 21)) );	/*Wait until the buffer is empty*/

    *ReceiveData = LPUARTx->DATA;
}

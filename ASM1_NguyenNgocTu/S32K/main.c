 ...
 *****************************************************************************/
/**
 * @brief    Define RED LED pin
 */
#define RED_LED                 15

/**
 * @brief    Define BLUE LED pin
 */
#define BLUE_LED                0

/**
 * @brief    Define GREEN LED pin
 */
#define GREEN_LED               16
 ...
 *****************************************************************************/
/**
 * @brief    Global variable to indicate Button state
 */
volatile uint8 g_u8ButtonState = 0;

/**
 * @brief    Global variable to indicate Led order (G -> R -> B)
 */
volatile uint8 g_u8LedOrder = 1;

/**
 * @brief    Global variable to indicate systick count
 */
volatile uint32 g_systick = 0;

/**
 * @brief    Global variable to indicate led state (green, red, blue)
 */
...
int main(void)
{
    /* Initialize reset value of global variables */
    g_u8ButtonState = 0;
    g_u8LedState = 0;
    g_u8LedOrder = 1;
    g_systick = 0;
    ...
    Interrupt_Enable(PORTA_IQRNumber, 0);
    Interrupt_Enable(PORTB_IQRNumber, 0);

    /* Config SysTick, 1 tick = 100ms */
